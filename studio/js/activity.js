...
function paint() {
  ...
  if(!jobs.length) list.append(node('div','activity-empty','No jobs running. Photo, video, voice, and music jobs appear here with where they run, their progress, and what they used.'));
...
if(originalFetch) window.fetch=async function trackedFetch(input,init={}) { const url=urlOf(input), method=String(init.method || (input instanceof Request?input.method:'GET')).toUpperCase(); let id='',category='';
  ...
  else if(method==='POST' && /:8189\/(upscale|tts|produce|layer|video\/)/.test(url) && !/\/video\/cancel(?:\?|$)/.test(url)){category='bridge'; const title=url.includes('/tts')?'Voice render':url.includes('/produce')?'Beat production':url.includes('/layer')?'Layer production':url.includes('/video/')?'Video job':'Photo upscale'; const suppliedId=headerOf(input,init,'X-MaterialLogix-Job-Id'); id=start({id:suppliedId||undefined,title,location:'local',status:'processing',detail:'Your computer',cancellable:Boolean(suppliedId&&url.includes('/video/'))});}
